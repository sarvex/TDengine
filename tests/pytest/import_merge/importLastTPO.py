...
class TDTestCase:
    ...
    def run(self):
        ...
        sqlcmd.extend('(%ld, %d)' % (startTime + rid, rid) for rid in range(1, 4))
        sqlcmd.extend('(%ld, %d)' % (startTime + rid, rid) for rid in range(6, 9))
        ...
        sqlcmd.extend('(%ld, %d)' % (startTime + rid, rid) for rid in range(1, 9))
        tdSql.execute(" ".join(sqlcmd))
