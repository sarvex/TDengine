...
class TDTestCase:
    ...
    def run(self):
        ...
        sqlcmd.extend('(%ld, %d)' % (startTime + rid, rid) for rid in range(1, 39))
        tdSql.execute(" ".join(sqlcmd))
        sqlcmd.extend('(%ld, %d)' % (startTime + rid, rid) for rid in range(40, 78))
        tdSql.execute(" ".join(sqlcmd))
        ...
        startTime = self.startTime
        sqlcmd = ['import into tb1 values']
        sqlcmd.extend('(%ld, %d)' % (startTime + rid, rid) for rid in range(39, 40))
        tdSql.execute(" ".join(sqlcmd))
